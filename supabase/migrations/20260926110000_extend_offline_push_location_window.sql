-- Browsers do not refresh volunteer geolocation while the installed PWA is closed.
-- Keep opted-in active volunteers eligible for trip push for seven days using their
-- last-known location, proximity, and route preference.
create or replace function public.get_nearby_volunteer_ids(
  p_trip_id uuid,
  p_radius_km double precision default 20,
  p_max_age_minutes integer default 10080
)
...
begin
  ...
  join public.volunteer_locations v
    on v.updated_at >= now() - make_interval(mins => greatest(coalesce(p_max_age_minutes, 10080), 1))
